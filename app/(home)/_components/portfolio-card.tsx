  ...
  return (
    <div className="group flex flex-col bg-white rounded-xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
      <div className="relative h-52 overflow-hidden bg-gray-100 rounded-t-xl">
        <div className="absolute inset-0 bg-center transition-transform duration-700 group-hover:scale-110 rounded-t-xl">
          <Image
            src={image}
            alt={title}
            fill
            sizes="(min-width: 1024px) 300px, (min-width: 768px) calc(50vw - 4rem), calc(100vw - 2rem)"
      ...
      <div className="p-6 flex flex-col flex-1 break-keep">
        <h3 className="text-lg font-bold text-text-main mb-2">{title}</h3>
        <p className="text-text-sub text-base leading-relaxed line-clamp-2">
          {description}
        </p>
      </div>
    </div>
